...
from PyQt5.QtGui import (
    QIcon, QPixmap, QColor, QLinearGradient, QPainter,
    QBrush, QPainterPath, QCursor, QFont, QDesktopServices,
)
...
class LyricsWindow(QMainWindow):
    ...
    _lyrics_ready = pyqtSignal(str, object)
    ...
    def __init__(self, config: dict, media_session, lyrics_fetcher):
        ...
        self._idle_message = "Play something\u2026"
        self._playback_inflight = False
        self._lyrics_request_track: str | None = None
        self._progress_anchor_ms = 0
        self._duration_ms = 0
        self._progress_anchor_ts = time.monotonic()
    ...
    def _start_polling(self):
        poll_interval = max(150, int(self.config.get("polling_interval_ms", 350)))
        self.poll_timer = QTimer(self)
        self.poll_timer.timeout.connect(self._request_playback_sample)
        self.poll_timer.start(poll_interval)

        self.sync_timer = QTimer(self)
        self.sync_timer.timeout.connect(self._render_interpolated_progress)
        self.sync_timer.start(50)

        QTimer.singleShot(50, self._request_playback_sample)

    def _request_playback_sample(self):
        if self._playback_inflight:
            return
    ...
    def _on_playback_ready(self, playback):
        self._playback_inflight = False
        ...
        if not playback:
            ...
            diag = ""
            if hasattr(self.media, "diagnostic_message"):
                try:
                    diag = (self.media.diagnostic_message() or "").strip()
                except Exception:
                    diag = ""
            self._idle_message = diag or "Play something\u2026"
            self._render_idle()
            self.title_bar.set_progress(0, 0)
            return
        ...
        self._apply_playback_sample(playback)
        ...
        if playback["track_key"] != self.current_track_key:
            ...
            self._set_gradient(DEFAULT_GRADIENT)
            self.media.fetch_thumbnail(playback["track_key"], self._on_thumbnail_ready)

            self._lyrics_request_track = playback["track_key"]
            duration_s = playback["duration_ms"] // 1000 if playback.get("duration_ms") else 0
        ...
        self.title_bar.set_playing(self._is_playing)
        self._render_interpolated_progress()

    def _fetch_lyrics_async(self, track_key: str, track_name: str, artist: str, album: str, duration_s: int):
        def _worker():
            ...
            self._lyrics_ready.emit(track_key, result)

        threading.Thread(target=_worker, daemon=True).start()

    def _on_lyrics_ready(self, track_key: str, lyrics):
        if track_key != self.current_track_key:
            return
        self.current_lyrics = lyrics or {"synced": False, "lines": []}
        self._render_lyrics()

    def _apply_playback_sample(self, playback: dict):
        now = time.monotonic()
        duration_ms = max(0, int(playback.get("duration_ms", 0) or 0))
        sample_ms = max(0, int(playback.get("progress_ms", 0) or 0))
        if duration_ms > 0:
            sample_ms = min(sample_ms, duration_ms)

        is_playing = bool(playback.get("is_playing", False))

        if playback["track_key"] != self.current_track_key:
            self._progress_anchor_ms = sample_ms
            self._duration_ms = duration_ms
            self._progress_anchor_ts = now
            self._is_playing = is_playing
            return

        predicted_ms = self.current_progress_ms()
        if is_playing:
            drift = sample_ms - predicted_ms
            if abs(drift) > 1800:
                # Seek/skip/restart happened: rebase immediately.
                self._progress_anchor_ms = sample_ms
            elif drift < -700:
                # Small regressions from noisy provider updates are softened.
                self._progress_anchor_ms = max(0, predicted_ms - 120)
            else:
                self._progress_anchor_ms = max(sample_ms, predicted_ms - 120)
        else:
            self._progress_anchor_ms = sample_ms
    ...
    def current_progress_ms(self) -> int:
        progress = self._progress_anchor_ms
        if self._is_playing:
            delta_ms = int((time.monotonic() - self._progress_anchor_ts) * 1000)
            if 0 <= delta_ms <= 10_000:
                progress += delta_ms
        if self._duration_ms > 0:
            progress = min(progress, self._duration_ms)
        return max(0, progress)

    def _render_interpolated_progress(self):
        progress_ms = self.current_progress_ms()
        self.title_bar.set_progress(progress_ms, self._duration_ms)
        if self.current_lyrics and self.current_lyrics.get("synced"):
            self._highlight_line(progress_ms)
    ...
    def _render_idle(self):
        self._clear_labels()
        idle = WordWrapLabel(self._idle_message)
    ...
    def _highlight_line(self, progress_ms: int):
        ...
        first_time = lines[0].get("time_ms", 0)
        if progress_ms < first_time:
